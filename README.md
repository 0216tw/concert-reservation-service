...
| RESERVATION |  1,300,000 | 콘서트 예약 완료 100만건 , 실패 30만건 
| PAYMENT |  1,000,000 | 콘서트 좌석 결제 성공 100만건 |
| USER | active횟수만큼 | 토큰 ACTIVE 전환된 시점 횟수만큼 USER 등록 | 
...
![image](https://github.com/user-attachments/assets/8494ee62-bfaf-478d-9a61-8d51fa408f75)

전체 데이터 수 : 50  <br> 
**인덱스 사용 유무 : X ( table full scan 사용 )** <br> 
이유 : <br> 
(1) 데이터 개수가 얼마 없다. <br> 
(2) 데이터 조회 비율이 전체 대비 5% 이상이다. **출력할 행의 수가 전체 행의 수 대비 5% 이상이라면 full scan 이 차라리 성능이 나음** <br> 
(3) 조회 당시 일자를 기준으로 시행되는 콘서트 목록만 불러와 조회 데이터 비율이 일정하다 <br> 
<br> 
실행계획 ( Cost = 2 ) <br> 
![image](https://github.com/user-attachments/assets/3edaf04f-1912-4e71-8ab2-5f03b355b7b5)
...
![image](https://github.com/user-attachments/assets/ddcc326a-9f46-49c7-bcbe-2c15bf16db1f)


전체 데이터 수 : 1500  <br> 
**인덱스 사용 유무 : O [ PK INDEX : CONCERT_ID + CONCERT_DY ]** <br> 
이유 : <br> 
(1) 조회시 CONCERT_ID 를 이용해 특정 ROW만 조회한다. <br> 
(2) 데이터 조회 비율이 전체 대비 2% 이다. (1500 건 중 30건)  **출력할 행의 수가 전체 행의 수 대비 5% 미만이라면 index range scan 이 좋다.** <br> 

<br> 

- 실행계획 1  (full scan을 이용한 경우 Cost = 4 ) <br> 
힌트를 활용해 강제로 full scan을 적용했다. 이 경우 테이블의 1500건 중 30건을 읽기 위해 약 4의 cost가 발생하였다. <br> 

![image](https://github.com/user-attachments/assets/d3687f9d-f084-487b-9618-be6dcf680623)

<br> 

- **실행계획 2 ( concert_id 로 index range scan을 사용한 경우 Cost = 2 ) PICK!!** 
<br> 
이 경우 장점이 두가지 더 발생한다. <br>

(1) SELECT 에 CONCERT_DY 만 있어 인덱스 컬럼인 [CONCERT_ID + CONCERT_DY] 를 충족, **테이블 스캔없이 인덱스만 스캔해도 된다** <br>
(2) 만약 CONCERT_DY 기준 정렬된 데이터가 필요하다고 해도, 인덱스를 활용하며 **자동정렬** 을 보장하므로, **정렬을 생략할 수 있다** <br>

![image](https://github.com/user-attachments/assets/d93681eb-9b96-42ab-89a0-591960f65e87)

정렬이 수반되는 경우 

![image](https://github.com/user-attachments/assets/2c88aea8-bb7d-480e-8f5f-82fd1bfb1e5c)
...
<쿼리 3 > 콘서트 예약 가능 좌석 조회 

![image](https://github.com/user-attachments/assets/de7c1c66-4d93-45dd-8f6c-05f983385548)

전체 데이터 수 : <br> 
Reservation Table 1,500,000 <br> 
Payment Table : 1,000,000 <br> 
ConcertSeat : 15,000,000 <br> 
<br> 

**인덱스 사용 유무 : O ( index range scan 활용 )** <br> 
이유 : <br> 
(1) 대용량 데이터를 가진 테이블끼리 **조인** 하므로 인덱스 활용이 필수라고 판단함 <br> 
(2) 자주 조회되는 쿼리이므로 cpu자원을 많이 소모하는 hash join 보다는 Nested Loop join이 더 적합하다고 판단함 <br> 
<br> 

사용 인덱스 : <br> 
ConcertSeat : pk index **[ concertId + concertDy ]** <br> 
Reservation : index **[ concertId + concertDy + seatNo ]** <br> 
Payment : index **[ reservationId ]** <br> 

<br> 

- 테스트1 . TABLE FULL SCAN으로 처리한 경우 (DB 내 소요시간 : 약 1초) <br> 

![image](https://github.com/user-attachments/assets/cb67e5d9-c475-4f36-927b-5de8e164a764)

<분석> <br> 
(1) 실행계획을 아래 안쪽 depth 부터 읽는다. <br> 
(2) PAYMENT 와 RESERVATION 을 각각 FULL SCAN 하면서 cost가 3 , 2 가 발생했다. <br> 
   HASH JOIN은 많은 자원을 사용하므로, 조회가 매우 자주 발생하는 OLTP 환경에서는 적합하지 못하다. <br> 
    
![image](https://github.com/user-attachments/assets/9e438f10-57e4-4b80-84af-e31b1d972e2c)

(3) 이후 CONCERT_SEAT 테이블과 해시 조인을 하는데, CONCERT_SEAT 가 1500만건의 대용량 테이블이라 COST가 약 16929 발생한 것을 볼 수 있다. <br> 
    따라서 이 쿼리를 table full scan 할 경우 발생하는 최종 cost는 16929가 발생하며, 조회 빈도 수 대비 매우 비효율적인 쿼리이다. <br> 

![image](https://github.com/user-attachments/assets/5a39cfa2-14b9-4415-ac8f-3bbc181e115a)

<br> 
<br> 

- 테스트2. 가장 문제가 되는 CONCERT_SEAT에 인덱스 추가 [ concert_id + concert_dy +_seat_no ] 한 경우 (DB 내 소요시간 : 약 0.464초) <br> <br>

![image](https://github.com/user-attachments/assets/eff51a60-8c94-440e-b6b3-2ec3e853e2fb)

이 전략의 핵심은 CONCERT_SEAT 의 조회조건 [ concert_id + concert_dy + seat_no ] 를 인덱스로 만들어 1500만건을 단지 1만건만 스캔할 수 있도록 하는 것이다. <br> 
적용 후 cost 는 다음과 같다. <br> 

![image](https://github.com/user-attachments/assets/c61f0e2d-19ac-4b47-8a4f-45bae22b1ea7)

**CONCERT_SEAT 라는 대용량 데이터를 full scan하는 데이터를 index를 적용함으로써 cost를 16929 에서 35로 감소** 시켰으며 <br> 
전체 cost 가 16929에서 52로 크게 감소하고, **시간은 1초 대에서 0.4초대**로 줄어든 것을 볼 수 있다. <br> 



---

<쿼리4> 예약 번호로 예약 정보 조회 <br> 

![image](https://github.com/user-attachments/assets/bc19b3eb-59fa-4405-82a9-380e52625063)


전체 데이터 수 : 1,500,000  <br> 
**인덱스 사용 유무 : O ( index unique scan )** <br> 
이유 : <br> 
(1) 데이터 개수가 많으며 이중 1개의 row 만 출력한다. <br> 
(2) 데이터 조회 비율이 전체 대비 1% 미만이다. <br> 
<br> 

--- 

<쿼리5> 콘서트 정보로 예약 정보 조회 <br> 

![image](https://github.com/user-attachments/assets/595f14dc-5af0-46cd-95e6-6137f5113c05)

전체 데이터 수 : RESERVATION 1,500,000 , PAYMENT 1,000,000 건 <br> 
**인덱스 사용 유무 : O ( index range scan )** <br> 
이유 : <br> 
(1) 대용량 테이블을 조인하므로 조건을 필터링 할 수 있는 index가 필요하다고 판단함 <br> 
(2) 조인 조건인 reservation_id 컬럼에 대해 효율적인 NL 조인이 가능하도록 PAYMENT 에 인덱스를 추가함 <br> 

- 테스트1 . TABLE FULL SCAN으로 처리한 경우 (DB 내 소요시간 : 약 0.007초) <br>

![image](https://github.com/user-attachments/assets/34d195f1-8769-49dd-ace7-dee3b73adca2)

최종 COST 가 3145 로, RESERVATION 테이블을 full scan 하면서 대부분의 cost가 발생했다. <br> 

- 테스트2 : RESERVATION 테이블에 인덱스 추가 [ concert_id  + concert_dy + seat_no ]
   
![image](https://github.com/user-attachments/assets/4663e550-daf9-4aec-b95a-fcf0c1da3756)

최종 COST가 8로, 기존 3145 대비 크게 COST를 줄일 수 있었다. <br> 
DB 내 소요기간도 기존 0.007 초에서 0.001 초로 줄었다. <br> 

---

기타 사용자 조회의 경우 user_id 를 인덱스로 주었습니다. (모든 조회 조건이 user_id 로 unique scan 처리) 
...
| 테이블 | index | 
| ------ | ----| 
| CONCERT |   pk ( concert_id)  | 
| CONCERT_SCHEDULE | pk ( concert_id + concert_dy)  | 
| CONCERT_SEAT |  pk (concert_id + concert_dy + seat_no )  |
| RESERVATION | pk ( reservation_id ) , idx1 ( concert_id + concert_dy + seat_no ) |
| PAYMENT |  pk (payment_id) , idx1 ( reservation_id) | 
| USER | pk (user_id) | 
